...
public class FirebaseHelper {
    ...
    public static List<User> convertToUserList(){
        ...
        for(int i=0 ; i<fromDBList.size() ; i++){
            User temp = new User(fromDBList.get(i).get("userName"),fromDBList.get(i).get("password"), fromDBNums.get(i).get("age"), fromDBList.get(i).get("email"),Math.toIntExact(fromDBNums.get(i).get("acecssLevel")));
            userList.add(temp);
        }
        return userList;
    }
